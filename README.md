...
- frontend, использующий образы node:24-alpine, nginxinc/nginx-unprivileged:1.30-alpine
- backend, использующий образы golang:1.25-alpine + alpine:3.22

Несмотря на легковесные образы, итоговый размер компонентов сервиса в развернутом состоянии довольно большой

yml конфиги для dev и prod включают конфигурирование эндпоинтов через переменные, healthcheck и перезапуски, поддержка
мастштабируемости
